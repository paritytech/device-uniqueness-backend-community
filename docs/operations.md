...
| A username appeared in search and then vanished | Expected, and rare: a speculative row whose block was discarded at the tip. `dub_indexer_speculative_retracted_total` counts them. It re-appears once the registration lands on the canonical chain. If the rate is high, the endpoint's fork rate is high — check `dub_chain_finality_trail_blocks`, or set `SPECULATIVE_INDEXING_ENABLED=false` to serve finalized state only. |
| `unfinalized window too wide; speculation standing down` | The best head has run more than 64 blocks ahead of finality, so the chain's finality is stalled rather than merely trailing. Speculation stands down and the finalized pass carries the projection alone; search goes back to finalized-only latency. Investigate finality on the chain, not the indexer. `dub_indexer_speculative_stood_down_total` counts it. |
